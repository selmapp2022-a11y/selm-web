/**
 *   npx tsc src/exam/engine/cell.t1.check.ts --outDir /tmp/c1 --module commonjs \
 *     --target es2020 --moduleResolution node --skipLibCheck --esModuleInterop
 *   node /tmp/c1/exam/engine/cell.t1.check.js
 */
import { TCF_CANADA } from '../definitions/tcf-canada';
import { ITEMS } from '../definitions/practice/tcf-ee-t1-nclc6.items';
import { THRESHOLDS, ITEM_PROXY_BOUNDS, FAILURE_MODE } from '../definitions/prescriptions/tcf-ee-t1-nclc6';
import { runGate } from './gate';
import { diagnoseEmptyMessage, factTokens } from './diagnose';
...
let fails = 0;
const notes: string[] = [];
console.log('CELL — TCF Canada · expression écrite · tâche 1 · NCLC 6');
console.log('items: ' + ITEMS.length + '   failure mode: ' + FAILURE_MODE.id);
console.log('thresholds: ' + JSON.stringify(THRESHOLDS) + '   band 60-120 mots\n');
...
for (const it of ITEMS) {
  ...
  const d7 = diagnoseEmptyMessage(task, it.responses.nclc7, THRESHOLDS, it.secondRequirement, SEG);
  const d6 = diagnoseEmptyMessage(task, it.responses.nclc6, THRESHOLDS, it.secondRequirement, SEG);
...
console.log(pad('item', 26) + pad('facts 7/6', 13) + pad('eval/100 7/6', 16) + 'says what was learned 7/6');
for (const it of ITEMS) {
  const task = taskFor(it);
  const d7 = diagnoseEmptyMessage(task, it.responses.nclc7, THRESHOLDS, it.secondRequirement, SEG);
  const d6 = diagnoseEmptyMessage(task, it.responses.nclc6, THRESHOLDS, it.secondRequirement, SEG);
  ...
  console.log(pad(it.id, 26) +
    pad(`${factTokens(it.responses.nclc7)} / ${factTokens(it.responses.nclc6)}`, 13) +
    pad(`${n1(g(d7, 'evaluative_density'))} / ${n1(g(d6, 'evaluative_density'))}`, 16) +
    `${g(d7, 'second_requirement')} / ${g(d6, 'second_requirement')}`);
}
