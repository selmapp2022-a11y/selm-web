...
import { Trophy, Star, Sparkles, ArrowRight, LayoutDashboard, Flame, TrendingUp } from 'lucide-react';
import clsx from 'clsx';
import { type ProgressEvent, type SkillKey, getSummary, recordProgress } from '../lib/progress';

type Props = {
  skill: SkillKey;
  topic?: string;
  score?: number;       // raw correct answers OR 0..100
  total?: number;       // out of (omit for free-form)
  // What pressing "Next" should do (e.g. load another exercise on the same page)
  onNext: () => void;
  nextLabel?: string;
  // Optional extra body shown above the buttons (e.g. feedback / badge etc)
...
export function CompletionCard({ skill, topic, score, total, onNext, nextLabel = 'Continue with another', extra }: Props) {
  // Record once per mount.
  const [recorded, setRecorded] = useState<{ ev: ProgressEvent; leveledUp: boolean; newLevel: number } | null>(null);
  ...
  useEffect(() => {
    ...
    setRecorded({ ev, leveledUp, newLevel: after.level });
      ...
      <div className={clsx(
        ...
        {recorded && (
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {recorded.leveledUp && (
              <span className="rounded-full bg-white px-3 py-1 text-xs font-bold text-orange-600 shadow-md">
                🎉 LEVEL UP!
              </span>
            )}
          </div>
        ...
        {after && (
          <div className="mt-4">
            <div className="mb-1 flex justify-between text-[11px] opacity-80">
              <span>{after.xpThisLevel} / 200 XP this level</span>
              <span>{after.xpToNext} to level {after.level + 1}</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-white/20">
              <div
                className="h-full rounded-full bg-white transition-all"
                style={{ width: `${(after.xpThisLevel / 200) * 100}%` }}
              />
            </div>
          </div>
        )}
